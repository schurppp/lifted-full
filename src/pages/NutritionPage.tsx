...
const NutritionPage: React.FC = () => {
  const [recipes, setRecipes] = useState<Recipe[]>(nutritionData as Recipe[]);
  ...
  const handleAddRecipe = () => {
    ...
    const recipe: Recipe = {
      id: 'custom-' + Date.now(),
      name: newRecipe.name,
      category: newRecipe.category as MealCategory,
      prepTimeMinutes: 0,
      ...
      <div className="nutrition-page">
        ...
        <div className="recipe-detail">
          ...
          <div className="macro-grid">
            <div className="macro-card"><span className="macro-value">{selected.calories}</span><span className="macro-label">kcal</span></div>
            <div className="macro-card"><span className="macro-value">{selected.protein}g</span><span className="macro-label">Protein</span></div>
            <div className="macro-card"><span className="macro-value">{selected.carbs}g</span><span className="macro-label">Kohlenhydrate</span></div>
            <div className="macro-card"><span className="macro-value">{selected.fat}g</span><span className="macro-label">Fett</span></div>
          </div>
          ...
          {(selected.steps ?? selected.instructions ?? []).length > 0 && (
            <div className="recipe-section">
              <h3>Zubereitung</h3>
              <ol>
                {(selected.steps ?? selected.instructions ?? []).map((s, i) => (
                  <li key={i}>{s}</li>
                ))}
              </ol>
            </div>
  ...
  return (
    <div className="nutrition-page">
      <div className="page-header">
        ...
        <button className="btn-outline" onClick={() => setShowAddForm(!showAddForm)}>+ Rezept hinzufügen</button>
      </div>
        ...
        <div className="add-recipe-form">
          ...
          <div className="form-group">
            ...
            <input
              ...
              onChange={e => setNewRecipe(p => ({ ...p, calories: e.target.value }))}
            />
          </div>
          <div className="form-group">
            ...
            <input
              ...
              onChange={e => setNewRecipe(p => ({ ...p, protein: e.target.value }))}
            />
          </div>
          <div className="form-group">
